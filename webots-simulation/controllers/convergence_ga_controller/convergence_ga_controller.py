...
strategy_f = open(f"../../graph-generation/collision-data/ga-info-{sim_type}-convergence.csv", 'a')
gene_df = open(f"../../graph-generation/collision-data/ga-gene-info-{sim_type}-convergence.csv", 'a')
...
def interpret(timestep): 
    ...
    if receiver.getQueueLength()>0:
        ...
        if message[0:2] == "#" + str(given_id):
            ...
            strategy_f.close()
            strategy_f = open(f"../../graph-generation/collision-data/ga-info-{sim_type}-{curr_sim_size}-convergence.csv", 'a')
            
            gene_df.write(str(given_id) + ','+ str(robot.getTime()) + ',' + str(trial_num) + ',' + str(curr_sim_size) + ',' + str(curr_robot_genotype) + '\n')
            gene_df.close()
            gene_df = open(f"../../graph-generation/collision-data/ga-gene-info-{sim_type}-{curr_sim_size}-convergence.csv", 'a')
